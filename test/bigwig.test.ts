...
test('loads variable step bigwig', async () => {
  const ti = new BigWig({
    path: 'test/data/variable_step.bw',
  })
...
test('loads simple fixedstep bigwig', async () => {
  const ti = new BigWig({
    path: 'test/data/volvox_microarray.bw',
  })
...
test('test uncompressed bw (-unc from wigToBigWig)', async () => {
  const ti = new BigWig({
    path: 'test/data/uncompressed.bw',
  })
